...
import { 
  ...
  ChevronDown, Filter, ArrowUpRight, ArrowDownRight, RefreshCw
} from 'lucide-react';
import { 
...
// --- FUNÇÃO DE DATA SEGURA (CORREÇÃO DE FUSO) ---
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function init() {
      try {
        await Promise.all([fetchInitialData(), fetchLiveDollar()]);
      } catch (e) {
        console.error("Erro fatal no init:", e);
      } finally {
        setLoading(false); // Garante que o loading saia
      }
    }
    init();
  ...
  const processedData = useMemo(() => {
    ...
    if (dateRange === 'today') { 
       ...
       // Custom usa string direta
    }

    // Usa a função segura ou a string direta do custom
          ...
          <div className="flex flex-wrap gap-4 items-center w-full xl:w-auto">
             ...
             <div className={`flex items-center p-1.5 rounded-lg border gap-4 ${bgCard}`}>
                <div className="flex gap-3 px-2 border-r border-inherit pr-4">
                   <div><span className="text-[9px] text-orange-500 uppercase font-bold block">Custo (Real)</span><span className="text-xs font-mono font-bold text-orange-400">R$ {liveDollar.toFixed(2)}</span></div>
